...
from django.utils.http import urlencode, timezone
...
class CandidateJobsView(APIView):
    ...
    def get_ads_per_page(self, total_jobs):
        """
        Dynamically calculate the number of ads to display based on the number of jobs.
        """
        config = GeneralSetting.objects.get_configuration()
        max_ads = config.ads_per_page  # Configured maximum ads per page

        # Ensure at least 3 jobs between two ads
        max_allowed_ads = max(0, (total_jobs - 1) // 3)  # Ads are limited to maintain 3 jobs between them

        # Return the minimum between configured max ads and allowed ads
        return min(max_ads, max_allowed_ads)

    def distribute_ads(self, jobs, ads, ads_per_page):
        """
        Distribute ads evenly among jobs, ensuring at least 3 jobs between ads.
        """
        ...
        # If ads exceed the number of allowed positions, cycle ads
        ads = ads * (ads_per_page // len(ads)) + ads[:ads_per_page % len(ads)]

        # Calculate the interval for inserting ads
        interval = max(1, total_jobs // ads_per_page)

        # Insert ads at calculated intervals
        ad_index = 0
        for i in range(interval, total_jobs + len(ads), interval + 1):
            if ad_index < len(ads):
...
class GetJobScoresByIdsView(APIView):
    ...
    def post(self, request):
        ...
        for job in jobs_to_compare:
            ...
            # Check if the score needs recalculating
            if job_search and job_search.last_scored_at and job_search.last_scored_at >= base_cv.cv_data.updated_at:
                continue
        ...
        if not jobs_data:
            return Response({"detail": "All jobs are up-to-date."}, status=status.HTTP_200_OK)
        ...
        try:
            ...
            for score_data in scores:
                ...
                # job = Job.objects.get(id=job_id)
                job = next((j for j in jobs_to_update if j.id == job_id), None)
                if job:
...
class GenerateJobCVScoreView(APIView):
    ...
    def post(self, request):
        ...
        # Check if a new score is needed
...
class CVDetailView(APIView):
    ...
    def put(self, request, id):
        ...
        request.data["cv_data"]["updated_at"] = datetime.now()
        # Update CVData
